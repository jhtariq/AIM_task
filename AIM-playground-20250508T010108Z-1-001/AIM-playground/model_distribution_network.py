...
class ModelDistributionNetwork(tf.Module):
  ...
  def compute_loss(self, images, actions, step_types, rewards=None, discounts=None, latent_posterior_samples_and_dists=None):
    ...
    posterior_latents = tf.concat([latent1_posterior_samples, latent2_posterior_samples], axis=-1)
    ...
    #adding 2 lines here to get the elbo loss
    elbo_per_sample = likelihood_log_probs - latent1_kl_divergences - latent2_kl_divergences
    loss = -tf.reduce_mean(elbo_per_sample)
    ...
    outputs.update({
      'elbo': tf.reduce_mean(elbo_per_sample),
      "log_likelihood": tf.reduce_mean(likelihood_log_probs),
      "kl_divergence": tf.reduce_mean(latent1_kl_divergences + latent2_kl_divergences),
      'images': images,
  ...
  def sample_prior_or_posterior(self, actions, step_types=None, images=None):
    ...
    for t in range(sequence_length + 1):
      is_conditional = images is not None and (t < images.shape[1])
      if t == 0:
        if is_conditional:
        ...
        reset_mask = tf.expand_dims(reset_mask, axis=-1)  # Make shape [4, 1]
        if is_conditional:
