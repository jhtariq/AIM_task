...
import nest_utils as slac_nest_utils
import gif_utils
...
class ActorSequencePolicy(tf_policy.TFPolicy):
  def __init__(self,
               time_step_spec=None,
               action_spec=None,
               info_spec=(),
  ...
  def _apply_actor_network(self, time_steps, actions, network_state):
    ...
    for actor_input in self._actor_input.split('__'):
      if actor_input == 'state':
        ...
        # tf.print("DEBUG: step_type =", time_steps.step_type)
        # tf.print("DEBUG: time_steps.observation =", time_steps.observation)
...
class SlacAgent(tf_agent.TFAgent):
  ...
  def model_loss(self,
    ...
    with tf.name_scope('model_loss'):
      ...
      with tf.name_scope('ModelOutputs'):
        for name, output in outputs.items():
          ...
          elif output.shape.ndims in [2, 3, 4]:  # Add this block!
            tf.summary.histogram(name, output, step=self.train_step_counter)
          else:
            raise NotImplementedError(f"Unsupported output shape: {output.shape}")
